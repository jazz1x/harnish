...
def _run(base_dir: "str | None") -> int:
    ...
    if event == "Stop":
        ...
        if pending_file.exists() and pending_file.stat().st_size > 0:
            ...
            try:
                from .promote import _cmd_promote as _promote
                import types
                ...
                # Capture promote output
                import io
                import contextlib
                buf = io.StringIO()
                with contextlib.redirect_stdout(buf):
                    _promote(fake_args)
                promote_raw = buf.getvalue().strip()
                try:
                    promote_data = json.loads(promote_raw)
                    promoted = promote_data.get("promoted", 0)
                    dedup = promote_data.get("deduplicated", 0)
                    if promoted > 0:
