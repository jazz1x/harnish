...
| **harnish** | `/harnish:harnish` | 자율 구현 엔진 (시딩 + ralph 루프 + 앵커링 + 경험축적) |
...
`.harnish/` 는 프로젝트 CWD 안에 위치하며 세션 간 유지된다. `harnish`, `drafti-architect`, `drafti-feature` 가 각 스킬의 Step 2에서 태그 기반으로 관련 자산을 자동 참조한다.
...
해당 스킬이 `forki` 로 호출 가능 (플러그인 네임스페이스 없음). `forki` 대신 `harnish`, `ralphi`, `drafti-architect`, `drafti-feature` 중 어느 것도 가능.
