...
| **ralph** | 0.0.1 | `/ralph` | 점검 (HITL 보고 또는 자율 수정) |
...
ralph  ──→  어떤 아티팩트든 점검 (PRD, SKILL.md, 스크립트, 코드)
...
스킬은 `/harnish:harnish`, `/harnish:drafti-architect`, `/harnish:drafti-feature`, `/harnish:ralph`로 등록됩니다.
...
- **ralph** = RALP (Recursive Autonomous Loop Process) + i (점검)
