...
| **drafti-feature** | 0.0.1 | `/drafti-feature` | 기획 기반 명세 PRD 생성 |
| **harnish** | 0.0.1 | `/harnish` | 자율 구현 엔진 (시딩 + RALP 루프 + 앵커링 + 경험축적) |
| **ralpi** | 0.0.1 | `/ralpi` | 아티팩트 정합성 검증 ("이 결과물이 올바른가?") |
...
ralpi  ──→  어떤 아티팩트든 검증 (PRD, SKILL.md, 스크립트, 코드)
```
...
→ drafti-feature가 피쳐플래그 설계 + 구현 명세 PRD 생성
...
```
사용자: "구현 시작" 또는 "태스크 분해"
→ PRD를 원자적 태스크로 분해 → PROGRESS.md 생성
→ "Phase 3개, Task 12개 시딩 완료 — 확인 후 '루프 돌려'"

사용자: "루프 돌려"
→ RALP 루프 자동 실행 (Read → Act → Log → Progress → repeat)
→ 매 3액션마다 PROGRESS.md 갱신, Phase 완료 시 마일스톤 보고

사용자: (새 세션에서) "이어서 진행"
→ PROGRESS.md에서 좌표 복원, 중단 지점부터 자동 재개
```

### 3. 검증 (ralpi)

```
사용자: "이 PRD 검증해" 또는 "/ralpi docs/prd-redis-cache.md"
→ 타입 감지 (PRD) → 정적 분석 → 이슈 발견 시 직접 수정 → 재검증
→ PASS 또는 FAIL (최대 7회 루프)

사용자: "src/cache.py 검증해"
→ 타입 감지 (코드) → 타입 체크 + lint + 테스트 실행 → 수정 → 재검증
→ 9개 언어 지원 (Python, TS, JS, Java, Kotlin, Go, Rust, Swift, Dart)
```
...
# mcpmarket
# drafti-architect, drafti-feature, harnish, ralpi 개별 설치
```
...
- **ralpi** = RALP (Recursive Autonomous Loop Process) + i (아티팩트 검증)
