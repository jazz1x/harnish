...
```
drafti-architect (또는 drafti-feature) → impl → galmuri:ralphi (자매 플러그인)
```
...
| galmuri:ralphi | 가능 (외부) | 검증 대상 파일/디렉토리 지정 |
...
세션 시작 이후 변경된 파일에 `galmuri:ralphi` (자매 플러그인) 점검 호출. 결과를 inline 보고. **자동 수정 금지** — 수정은 별개 user trigger. galmuri 미설치 시 이 단계를 건너뛰고 한 줄 안내만 표시 ("galmuri 미설치 — 완료 후 점검 생략").

```
📋 완료 후 점검 (galmuri:ralphi, 읽기 전용)
이 세션에서 변경된 파일: {N}개
발견: {심각도별 개수}
```
...
다음 제안: {예: galmuri:ralphi 수정 | 배포 | 인계 | 다른 플러그인 호출}
```
...
| Step 6 (완료 의례) | galmuri:ralphi 출력 + compress-assets dry-run 출력 + 세션 자산 |
