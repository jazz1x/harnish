...
- 디렉토리가 주어짐 → **디렉토리 스코프** → Step 3B (범위: 해당 디렉토리 내 `git diff HEAD~3`)
- 아무것도 없음 → **프로젝트 스코프** → Step 3B (범위: `git diff --name-only HEAD~3`)

변경 파일 없으면 사용자에게 범위를 묻는다. **추측 금지.**
...
1. 테스트 실행. 코드를 읽기 전에 테스트부터. 테스트 러너 모르면 **사용자에게 질문. 추측 금지.**
2. 변경 파일 목록 (git diff)
